...
export function TasksClient({ tasks, staff }: { tasks: TaskDTO[]; staff: StaffDTO[] }) {
  ...
  const filtered = useMemo(() => {
    return tasks.filter((t) => {
      ...
      if (owner !== "all" && t.owner.reference !== owner) return false;
      if (search) {
  ...
  }, [tasks, status, category, owner, search]);

  const columns = useMemo<ColumnDef<TaskDTO, unknown>[]>(
    () => [
      selectionColumn<TaskDTO>("tasks"),
      {
        accessorKey: "title",
        header: "Task",
        cell: ({ row }) => (
  ...
  return (
    <div className="mx-auto max-w-[100rem]">
      ...
      <NewTaskDialog staff={staff} />
    </div>
  );
}

function NewTaskDialog({ staff }: { staff: StaffDTO[] }) {
  const [title, setTitle] = useState("");
  const [category, setCategory] = useState("Call");
  const [priority, setPriority] = useState("medium");
  const [ownerReference, setOwnerReference] = useState(staff[0]?.reference ?? "");
  const [pending, startTransition] = useTransition();

  return (
    <ParamDialog
      title="New task"
      description="Track a piece of work."
      submitLabel={pending ? "Creating…" : "Create task"}
      onSubmit={() => {
        if (!title.trim()) {
        ...
        startTransition(async () => {
          const result = await createTask({
            ...
            category,
            ownerReference,
            // Due today by default. The dialog has no date field; adding one
            // is a design change rather than part of this migration.
          ...
          setTitle("");
          toast("Task created", { description: result.data.reference });
        });
      }}
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Owner" htmlFor="tk-owner">
          <Select value={ownerReference} onValueChange={setOwnerReference}>
            <SelectTrigger id="tk-owner" className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {staff
