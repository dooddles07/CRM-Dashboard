...
const sourceOptions = Object.entries(sourceLabels) as [string, string][];
...
/** Runs once, inside `useState`'s lazy initializer — not on every render. */
function buildEmptyDraft(departments: DepartmentOption[], doctors: DoctorOption[]): Draft {
  const firstDepartment = departments[0]?.id ?? "";
  // Scoped strictly to firstDepartment — falling back to doctors[0] (a
  // doctor sorted first system-wide) can pair a department with a doctor
  // from a different one when the first department has none of its own.
  const firstDoctor = doctors.find((d) => d.departmentId === firstDepartment);
  ...
  return {
    ...
    doctorReference: firstDoctor?.reference ?? "",
...
export function NewPatientClient({ departments, doctors }: NewPatientClientProps) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const [step, setStep] = useState(1);
  const [draft, setDraft] = useState<Draft>(() => buildEmptyDraft(departments, doctors));
    ...
    startTransition(async () => {
      try {
        const result = await createPatient({
          ...
          emergencyContact:
            draft.emergencyName.trim() || draft.emergencyRelation.trim() || draft.emergencyPhone.trim()
              ? {
          ...
          doctorReference: draft.doctorReference || null,
          source: draft.source,
        ...
        toast("Patient created", {
          description: `${draft.name.trim()} · ${result.data.reference} · contact details are masked by default.`,
        });
        router.push(`/patients/${result.data.reference}`);
      } catch {
        // A rejected transition routes to error.tsx by default, whose copy
        // describes a failed *load* ("department and doctor lists did not
        // come back") — wrong for a failed *submit*, and it would remount
        // the form, discarding all three steps. Caught here so a stale
        // deploy or dropped connection reads as a retryable submit failure
        // instead.
        toast.error("That could not be saved. Check your connection and try again.");
      }
    });
  }
            ...
            <>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      set("doctorReference", first?.reference ?? "");
                    }}
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={draft.doctorReference}
                    onValueChange={(v) => set("doctorReference", v)}
                  >
            ...
            <>
              <dl className="divide-y divide-line rounded-md border border-line">
                {[
                  ...
                  ["Gender", draft.gender],
                  ["Department", departments.find((d) => d.id === draft.departmentId)?.name ?? "—"],
                  [
                    "Assigned doctor",
                    doctors.find((d) => d.reference === draft.doctorReference)?.name ?? "—",
                  ],
                  ["Preferred channel", draft.preferredChannel.toUpperCase()],
                  ["Source", sourceLabels[draft.source as keyof typeof sourceLabels] ?? draft.source],
