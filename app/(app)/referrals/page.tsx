...
import { referrals } from "@/lib/data/patient-record";
  ...
  const columns = useMemo<ColumnDef<Referral, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Patient",
        cell: ({ row }) => <span className="whitespace-nowrap font-medium text-ink">{row.original.patientName}</span>,
      },
      {
      ...
      },
      {
        id: "department",
        header: "Department",
        accessorFn: (r) => departmentName(r.departmentId),
      },
      {
        accessorKey: "status",
        header: "Status",
      ...
      },
      {
        id: "owner",
        header: "Owner",
        accessorFn: (r) => staffById(r.ownerId)?.name ?? "",
      },
      {
