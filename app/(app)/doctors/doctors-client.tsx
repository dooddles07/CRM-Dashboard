...
import type { DoctorDTO } from "@/lib/server/services/directory";
...
export function DoctorsClient({ doctors, departments }: { doctors: DoctorDTO[]; departments: { id: string; name: string }[] }) {
  ...
  const filtered = useMemo(() => {
    return doctors.filter((d) => {
      if (department !== "all" && (d.department?.id ?? "") !== department) return false;
          ...
          {filtered.map((d) => (
            ...
            >
              <div className="flex items-start gap-3">
                <PersonAvatar name={d.name} id={d.reference} size="lg" initials={d.initials} />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-caption text-ink-3">{d.department?.name ?? "Unassigned"}</p>
                </div>
              </div>
              ...
              <div className="mt-2 flex items-center justify-between text-caption">
                ...
                <StatusChip
                  meta={{ label: d.noShowRate === null ? "—" : `${d.noShowRate}%`, tone: noShowRisk(d.noShowRate ?? 0), icon: Stethoscope }}
                  showIcon={false}
                />
              </div>
            </Link>
