...
| 10 | `0009_fix_table_ownership.sql` | Step 2 | **Neon-provisioned role**, not `careflow_owner` | Repair. Phase 02's migrations ran as the Neon role, so nine auth tables ended up owned by it with no grant to `careflow_app` — `0005` cannot even be re-run until this fixes it (`permission denied for table verification`). Run this **before** re-running `0005`, then `0007`, then `0008`. |
